      ...
      <header className={`w-full sticky top-0 z-50 border-b border-gray-200 transition-all duration-300 ${isScrolled ? 'bg-white/75 backdrop-blur-lg shadow-md' : 'bg-white shadow-sm'}`}>
        <div className="container mx-auto px-4 py-2 relative flex items-center justify-between">
          ...
          <div className="hidden lg:flex flex-col text-right justify-center h-8">
            <span className="text-xs lg:text-sm text-gray-600 tracking-wide leading-tight select-none">Have Questions? We're Here To Help</span>
            <span className="text-sm lg:text-base text-[#f97316] font-bold tracking-wide leading-tight select-none">Call : +91-9314444747</span>
          </div>
        </div>
      </header>
    );
  ...
  return (
    <>
      <header className={`w-full sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 backdrop-blur-lg shadow-md' : 'bg-white shadow-sm'}`}>
        ...
        <div className="container mx-auto px-2 md:px-4 lg:px-6 py-[0.45rem] lg:py-2 relative flex items-center justify-between">
          <Link
            to="/"
            className="group z-10 flex items-center gap-1.5 md:gap-3 lg:gap-3 outline-none focus:outline-none focus:ring-0 border-none select-none"
            style={{ WebkitTapHighlightColor: 'transparent' }}
          >
            ...
            <h1 className="text-[clamp(1.1rem,4vw,1.5rem)] font-bold tracking-normal whitespace-nowrap">
          ...
          <form onSubmit={handleDesktopSearchSubmit} className="hidden md:flex flex-1 md:max-w-md lg:max-w-2xl md:mx-4 lg:mx-8 md:h-9 lg:h-10">
            <input
              ...
              className="flex-1 h-full bg-gray-100 text-sm md:text-sm lg:text-base text-gray-800 outline-none rounded-l-md px-4 focus:outline-none focus:ring-0 transition-all border-none"
            />
          ...
          <div className="flex items-center z-20">
            <div className="flex md:hidden items-center gap-1">
              <button onClick={toggleMobileSearch} className="p-1.5 flex items-center justify-center rounded-full text-gray-600 hover:text-[#f97316] transition-colors duration-300 outline-none focus:outline-none focus:ring-0 active:outline-none border-none shadow-none bg-transparent" style={{ WebkitTapHighlightColor: 'transparent' }}>
                <Search size={18} />
              </button>
              <Link to={isAuthenticated ? "/account" : "/login"} className="p-1.5 flex items-center justify-center rounded-full text-gray-600 hover:text-[#f97316] transition-colors duration-300 outline-none focus:outline-none focus:ring-0 active:outline-none border-none shadow-none bg-transparent" style={{ WebkitTapHighlightColor: 'transparent' }}>
                <User size={18} className={isAuthenticated ? "text-[#f97316]" : ""} />
              </Link>
            </div>
            ...
            <div className="hidden md:flex items-center gap-1 md:gap-2 lg:gap-3 flex-shrink-0">
              ...
              <Link to="/wishlist" className="w-10 h-10 md:w-10 md:h-10 lg:w-11 lg:h-11 flex items-center justify-center rounded-full text-gray-600 hover:bg-orange-50 hover:text-[#f97316] transition-all duration-300 outline-none focus:outline-none focus:ring-0 border-none shadow-none" title="Wishlist" style={{ WebkitTapHighlightColor: 'transparent' }}>
                <div className="relative flex items-center justify-center">
                  ...
                  {wishlistCount > 0 && (
                    <span className="absolute -top-1.5 -right-2 bg-[#f97316] text-white text-xs font-bold h-4 w-4 md:h-5 md:w-5 lg:h-6 lg:w-6 rounded-full flex items-center justify-center border border-white">
        ...
        <div className={`md:hidden absolute top-full left-0 w-full bg-white border-t border-gray-100 shadow-md p-2 transition-all duration-300 origin-top ${isSearchActive ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0 pointer-events-none'}`}>
          <form onSubmit={handleMobileSearchSubmit} className="flex items-center h-9 w-full">
            <input ref={mobileSearchInputRef} type="text" placeholder="Search for furniture..." className="flex-1 h-full bg-gray-100 text-sm text-gray-800 outline-none rounded-l-md px-4 focus:outline-none focus:ring-0 border-none" />
